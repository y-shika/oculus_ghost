...
//extern void start(float x, float y, float z);

extern int va_canny_arg1;
...
void start(ovrHmd hmd)
{
	...
	OVRManager.GetAcceleration(0, ref m_a[0], ref m_a[1], ref m_a[2]);
}
*/
...
static bool MainLoop(bool retryCreate)
{
	...
	if (ovrvision.Open(locationID, cameraMode)) {
		...
		va_init(width, height, pixelsize);
	}

	cv::namedWindow("XY", CV_WINDOW_AUTOSIZE);
	cv::namedWindow("XZ", CV_WINDOW_AUTOSIZE);
	cv::namedWindow("YZ", CV_WINDOW_AUTOSIZE);

	graph1 = cv::Mat(500, 500, CV_8UC3, cv::Scalar(255, 255, 255));
	graph2 = cv::Mat(500, 500, CV_8UC3, cv::Scalar(255, 255, 255));
	graph3 = cv::Mat(500, 500, CV_8UC3, cv::Scalar(255, 255, 255));

	cv::line(graph1, cv::Point(250, 0), cv::Point(250, 500), cv::Scalar(0, 0, 0), 1, 1);
	cv::line(graph1, cv::Point(0, 250), cv::Point(500, 250), cv::Scalar(0, 0, 0), 1, 1);
	cv::line(graph2, cv::Point(250, 0), cv::Point(250, 500), cv::Scalar(0, 0, 0), 1, 1);
	cv::line(graph2, cv::Point(0, 250), cv::Point(500, 250), cv::Scalar(0, 0, 0), 1, 1);
	cv::line(graph3, cv::Point(250, 0), cv::Point(250, 500), cv::Scalar(0, 0, 0), 1, 1);
	cv::line(graph3, cv::Point(0, 250), cv::Point(500, 250), cv::Scalar(0, 0, 0), 1, 1);
	...
	while (DIRECTX.HandleMessages())
	{
		...
		*/
		...
		/*
		if (DIRECTX.Key['1']) {
			va_canny_arg1 = 10; va_canny_arg2 = 20;
		}
		if (DIRECTX.Key['2']) {
			va_canny_arg1 = 20; va_canny_arg2 = 40;
		}
		if (DIRECTX.Key['3']) {
			va_canny_arg1 = 40; va_canny_arg2 = 80;
		}
		if (DIRECTX.Key['4']) {
			va_canny_arg1 = 80; va_canny_arg2 = 160;
		}
		if (DIRECTX.Key['5']) {
			va_canny_arg1 = 160; va_canny_arg2 = 320;
		}
